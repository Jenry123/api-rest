...
exports.agregarMaquinaria =[authenticateJWT, (req, res) => {
  const { id_maquinaria, nombre, descripcion, cantidad,id_inventario } = req.body;
...
exports.actualizarMaquinaria = [authenticateJWT,(req, res) => {
  ...
  const updateMaquinaria= req.body;
  db.query('UPDATE Maquinaria SET ? where id_maquinaria=?',
    [idMaquinaria,updateMaquinaria],
    (err, result) => {
      if (err) {
